...
// The shared setup replaces navigator; the real SDK also needs browser identification.
vi.hoisted(() => {
	vi.stubGlobal('navigator', { ...navigator, userAgent: 'Mozilla/5.0', appVersion: '5.0' });
});
...
describe('ArcGISMapProvider', () => {
	...
	test.each([true, false])('preserves walking dashes with arrows %s', async (withArrow) => {
		...
		const layers = route.symbol.data.symbol.symbolLayers;
		expect(layers.find((layer) => layer.type === 'CIMSolidStroke')).toMatchObject({
			type: 'CIMSolidStroke',
			width: 5,
			color: [18, 52, 86, 127.5],
			effects: [{ type: 'CIMGeometricEffectDashes', dashTemplate: [8, 12] }]
		});
		expect(layers.find((layer) => layer.type === 'CIMVectorMarker').enable).toBe(withArrow);
	});
	...
	test('converts real Web Mercator extents to WGS84 and handles unavailable bounds', async () => {
		const provider = await initializedProvider();
		const geographic = new Extent({
			xmin: -122.4,
			ymin: 47.5,
			xmax: -122.2,
			ymax: 47.7,
		...
		provider.view.extent = geographicToWebMercator(geographic);
		const bounds = provider.getBoundingBox();
		expect(bounds).not.toBeNull();
		expect(bounds.north).toBeCloseTo(47.7);
		expect(bounds.south).toBeCloseTo(47.5);
		expect(bounds.east).toBeCloseTo(-122.2);
		expect(bounds.west).toBeCloseTo(-122.4);
		expect(sdk.projection.project).not.toHaveBeenCalled();
		provider.view.extent = geographic;
	...
	test.each([0, 0.5, 1])('SDK reads route opacity %s and arrow dimensions', async (opacity) => {
		const provider = await initializedProvider();
		const route = provider.createPolyline(SHAPE, { color: '#123456', opacity });
		const color = getCIMSymbolColor(route.symbol);
		expect(color.a).toBe(opacity);
		expect(getCIMSymbolSize(route.symbol)).toBeGreaterThan(0);
		const arrow = route.symbol.data.symbol.symbolLayers[0];
		expect(arrow).toMatchObject({
			type: 'CIMVectorMarker',
			enable: true,
			size: 12,
			frame: { xmin: -6, ymin: -6, xmax: 6, ymax: 6 },
			markerPlacement: { angleToLine: true }
		});
		expect(arrow.markerGraphics[0].geometry.rings[0]).toEqual([
			[6, 0],
			[-6, -6],
			[-6, 6],
			[6, 0]
		]);
		expect(arrow.markerGraphics[0].symbol.symbolLayers[0].color[3]).toBe(opacity * 255);
		const plain = provider.createPolyline(SHAPE, {
			color: '#123456',
			opacity,
			withArrow: false,
			casing: true
		});
		expect(plain.symbol.color.toRgba()).toEqual([18, 52, 86, opacity]);
		expect(plain._casing.symbol.color.a).toBe(0.95);
	});
	...
	test('creates synchronous, WGS84-converted polylines with opacity, casing, and arrows', async () => {
		...
		expect(polyline.symbol.data.symbol.symbolLayers).toHaveLength(2);
